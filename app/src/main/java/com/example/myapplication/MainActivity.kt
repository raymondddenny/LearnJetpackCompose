...
fun GreetingImage(name: String, from: String, modifier: Modifier = Modifier) {
    val imageParty = painterResource(id = R.drawable.androidparty)

    Box(modifier = modifier) {
        Image(
            painter = imageParty, contentDescription = "Image Party",
            contentScale = ContentScale.Crop,
            alpha = 0.5F

        )
        GreetingText(
            name = name, from = from, modifier = Modifier
                .fillMaxSize()
                .padding(16.dp)
        )
...
//        Greeting("Android")
        GreetingText(
            name = stringResource(id = R.string.happy_birthday_text),
